package exhibition.pathfinding;
...
//		System.out.println(to.toString());
    ...
    public static TeleportResult pathFinderTeleportBack(ArrayList<Vec3> positions) {
        ...
        for (int i = positions.size() - 1; i > -1; i--) {
            //mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(positions.get(i).xCoord, positions.get(i).yCoord, positions.get(i).zCoord, false));
...
//	}
    ...
    public static boolean validEntity(EntityLivingBase en) {
        ...
        if (en instanceof EntityPlayer && Client.getModuleManager().isEnabled(Freecam.class)
                && en.getName().equals(Minecraft.getMinecraft().thePlayer.getName())) {
            return false;
        }
        if (en instanceof EntityPlayer && Client.getModuleManager().isEnabled(Blink.class)
                && en.getName().equals(Minecraft.getMinecraft().thePlayer.getName())) {
            return false;
        }
        if (en.isDead) {
