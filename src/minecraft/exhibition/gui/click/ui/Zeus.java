...
import static exhibition.util.RenderingUtil.drawIcon;
...
/*        for(int x = 5; x < 397; x+=2) {
        ...
        RenderingUtil.glColor(Colors.getColor(c.getRed(), c.getGreen(), c.getBlue(), (int) (c.getAlpha())));
    ...
    public void categoryButtonMouseClicked(CategoryButton p0, MainPanel p1, int p2, int p3, int p4) {
        if (p2 >= p0.x + p1.dragX && p3 >= p1.dragY + p0.y && p2 <= p1.dragX + p0.x + Client.f.getWidth(p0.name.toUpperCase()) + 5 && p3 <= p1.dragY + p0.y + 12 && p4 == 0) {
            int i = 0;
    ...
    public void categoryButtonDraw(CategoryButton p0, float p2, float p3) {
        ...
        if (p2 >= p0.x + p0.panel.dragX && p3 >= p0.y + p0.panel.dragY && p2 <= p0.x + p0.panel.dragX + Client.f.getWidth(p0.name.toUpperCase()) + 5 && p3 <= p0.y + p0.panel.dragY + 12 && !p0.enabled) {
...
/*            if (p0.enabled) {
            ...
            for (Module module : Client.getModuleManager().getArray()) {
                if (module.getType() == ModuleData.Type.Combat) {
                    ...
                    if (getSettings(module) != null) {
                        ...
                        List<Setting> listToAdd = new ArrayList<>();
                        for (Setting setting : getSettings(module)) {
                            if (setting.getValue() instanceof Options) {
                                listToAdd.add(setting);
                                if (yOff + y >= biggestY) {
                                    biggestY = 20 + categoryButton.panel.y + 16;
                                }
                            }
    ...
    public void categoryPanelDraw(CategoryPanel categoryPanel, float x, float y) {
        if (categoryPanel.visible) {
            RenderingUtil.rectangle(114 + categoryPanel.categoryButton.panel.dragX, 65 + categoryPanel.categoryButton.panel.dragY, 115 + categoryPanel.categoryButton.panel.dragX + Client.fs.getWidth(categoryPanel.headerString) + 2, 65 + categoryPanel.categoryButton.panel.dragY + Client.fs.getWidth(categoryPanel.headerString), Colors.getColor(17));
    ...
    public void buttonDraw(Button p0, float p2, float p3, CategoryPanel panel) {
        if (panel.categoryButton.enabled) {
            ...
            Client.fs.drawString(p0.module.getName(), (p0.x + xOff + 1), (p0.y + 2.5f + yOff - 7), -1);
            String meme = !p0.module.getKeybind().getKeyStr().equalsIgnoreCase("None") ? "Enable [" + p0.module.getKeybind().getKeyStr() + "]" : "Enable";
            Client.fss.drawString(meme, (p0.x + 7.6f + xOff), (p0.y + 2.7f + yOff), p0.isBinding ? Colors.getColor(ColorManager.hudColor.red, ColorManager.hudColor.green, ColorManager.hudColor.blue, 255) : p0.enabled ? -1 : Colors.getColor(180));
            ...
            if (hovering) {
                ...
                float width = Client.fs.getWidth("Desc: ");
    ...
    public void checkBoxDraw(Checkbox p0, float p2, float p3, CategoryPanel panel) {
        if (panel.categoryButton.enabled) {
            ...
            Client.fs.drawStringWithShadow("", 0, 0, Colors.getColor(0, 0));
            Client.fss.drawString(xd, (p0.x + 7.5f + xOff), (p0.y + 2.6f + yOff), (hovering ? Colors.getColor(ColorManager.hudColor.red, ColorManager.hudColor.green, ColorManager.hudColor.blue, 255) : p0.enabled ? -1 : Colors.getColor(180)));
            GlStateManager.popMatrix();
            if (p0.enabled) {
                RenderingUtil.drawGradient(p0.x + xOff + 1, p0.y + yOff + 1, p0.x + xOff + 5, p0.y + yOff + 5, Colors.getColor(ColorManager.hudColor.red, ColorManager.hudColor.green, ColorManager.hudColor.blue, 255), Colors.getColor(ColorManager.hudColor.red, ColorManager.hudColor.green, ColorManager.hudColor.blue, 120));
            }
    ...
    public void dropDownDraw(DropdownBox p0, float p2, float p3, CategoryPanel panel) {
        ...
        Client.fs.drawStringWithShadow("", 0, 0, Colors.getColor(0, 0));
        Client.fss.drawString(p0.option.getName(), (p0.x + xOff), (p0.y - 5 + yOff), -1);
        GlStateManager.pushMatrix();
        ...
        GlStateManager.popMatrix();
        Client.fss.drawString(p0.option.getSelected(), (p0.x + 2 + xOff) - 1, (p0.y + 2.5f + yOff), -1);
        if (p0.active) {
...
//        RenderingUtil.drawGradient(p0.x +  xOff, p0.y + yOff, p0.x  + xOff + 40, p0.y + yOff + 6, Colors.getColor(46), Colors.getColor(27));
        ...
        Client.fs.drawStringWithShadow("", 0, 0, Colors.getColor(0, 0));
        Client.fss.drawString(p0.name, (p0.x + 1 + xOff), (p0.y + 2.5f + yOff), hovering ? Colors.getColor(ColorManager.hudColor.red, ColorManager.hudColor.green, ColorManager.hudColor.blue, 255) : -1);
    ...
    public void colorPrewviewDraw(ColorPreview colorPreview, float x, float y) {
        ...
        RenderingUtil.rectangle(xOff - 74, yOff - 6, xOff - 73 + Client.fs.getWidth(colorPreview.colorName) + 1, yOff - 4, Colors.getColor(17));
    ...
    public void SliderDraw(Slider slider, float x, float y, CategoryPanel panel) {
        if (panel.visible) {
            ...
            Client.fs.drawString(valu2e, slider.x + xOff + textX, (slider.y + yOff + 2.6f), -1);
            GlStateManager.scale(1, 1, 1);
            GlStateManager.popMatrix();
            Client.fss.drawString(xd, (slider.x + xOff), (slider.y - 5 + yOff), -1);
