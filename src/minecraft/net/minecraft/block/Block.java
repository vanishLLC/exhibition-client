...
public class Block {
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        if (x.isEnabled() && !((boolean) x.getSettings().get("CAVE").getValue())) {
        ...
        if (module.isEnabled()) {
            ...
            if (zombieAim.usingAutowall() && zombieAim.showAutowall()) {
                ...
                for (Block penetrableBlock : penetrableBlocks) {
                    if (worldIn.getBlockState(pos).getBlock() == penetrableBlock) {
                        breakLoop = true;
                        break;
                    }
                }
                ...
                if (!breakLoop)
                    for (Block penetrableBlock : penetrableBlocks) {
                        if (worldIn.getBlockState(pos.add(0, 1, 0)).getBlock() == penetrableBlock) {
                            return true;
                        }
                        if (worldIn.getBlockState(pos.add(0, -1, 0)).getBlock() == penetrableBlock) {
                            return true;
                        }
                        if (worldIn.getBlockState(pos.add(1, 0, 0)).getBlock() == penetrableBlock) {
                            return true;
                        }
                        if (worldIn.getBlockState(pos.add(-1, 0, 0)).getBlock() == penetrableBlock) {
                            return true;
                        }
                        if (worldIn.getBlockState(pos.add(0, 0, 1)).getBlock() == penetrableBlock) {
                            return true;
                        }
                        if (worldIn.getBlockState(pos.add(0, 0, -1)).getBlock() == penetrableBlock) {
                            return true;
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity) {
        ...
        ebb.fire(this, pos, axisalignedbb);
        if (collidingEntity == Minecraft.getMinecraft().thePlayer) {
            axisalignedbb = ebb.getBounds();
        }
