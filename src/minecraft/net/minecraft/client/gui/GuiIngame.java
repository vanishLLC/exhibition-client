...
public class GuiIngame extends Gui {
    ...
    protected String getSaltString(String originalForSalt, int needed, boolean numbers) {
        if (needed <= 0) {
            return "";
        }
        String SALTCHARS = numbers ? "0123456789" : "GHIJKLMNOPQRSTUVWXYZ" + "ghijklmnopqrstuvwxyz";
        StringBuilder salt = new StringBuilder();
        Random rnd = new Random();
        while (salt.length() < needed) { // length of the random string.
...
//                try {
//                    String[] bruh = s1.split(" ");
//                    boolean lowercase = s1.contains("m");
//                    String matchNumber = Integer.toHexString(AuthenticationUtil.authListPos);
//                    if(matchNumber.length() < 3) {
//                        matchNumber += getSaltString(s1, 3 - matchNumber.length(), false);
//                    }
//                    matchNumber += getSaltString(s1, 1, true);
//                    String spoofedString = "\2478" + (lowercase ? "m" : "M") + matchNumber;
//                    s1 = bruh[0] + "  " + spoofedString;
//                } catch (Exception e) {
//
//                }
//            }
