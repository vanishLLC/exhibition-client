    ...
    public void renderItemModelForEntity(ItemStack stack, EntityLivingBase entityToRenderFor, ItemCameraTransforms.TransformType cameraTransformType)
    {
        if (stack != null && entityToRenderFor != null)
        {
            ...
            if (entityToRenderFor instanceof EntityPlayer)
            {
                ...
                } else if(item instanceof ItemSword && Minecraft.getMinecraft().gameSettings.thirdPersonView != 0) {
                    try {
                        if (entityplayer.getItemInUseCount() > 0 && entityplayer.getHeldItem().getItemUseAction() == EnumAction.BLOCK) {
                            GL11.glTranslatef(-0.1F, entityplayer.isSneaking() ? 0.075F : 0.15F, -0.05F);
