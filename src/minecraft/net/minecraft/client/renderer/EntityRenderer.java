...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float p_78473_1_) {
        Entity mcRenderEntity = mc.getRenderViewEntity();
        if ((mcRenderEntity != null) && (mc.theWorld != null)) {
            mc.mcProfiler.startSection("pick");
            mc.pointedEntity = null;
            double reach = mc.playerController.getBlockReachDistance();
            mc.objectMouseOver = mcRenderEntity.rayTrace(reach, p_78473_1_);
            double maxReach = reach;
            Vec3 attackerVec = mcRenderEntity.getPositionEyes(p_78473_1_);
            ...
            if (this.mc.objectMouseOver != null) {
                maxReach = this.mc.objectMouseOver.hitVec.distanceTo(attackerVec);
            }
            final Vec3 lookVec = mcRenderEntity.getLook(p_78473_1_);
            final Vec3 expandedVec = attackerVec.addVector(lookVec.xCoord * reach, lookVec.yCoord * reach, lookVec.zCoord * reach);
            this.pointedEntity = null;
            Vec3 hitVec = null;
            final float one = 1.0f;
            final List<Entity> entitiesWithinBoundingBox = this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(mcRenderEntity, mcRenderEntity.getEntityBoundingBox().addCoord(lookVec.xCoord * reach, lookVec.yCoord * reach, lookVec.zCoord * reach).expand((double) one, (double) one, (double) one));
            double reachTemp = maxReach;
            for (Entity entityInBox : entitiesWithinBoundingBox) {
                if (entityInBox.canBeCollidedWith()) {
                    ...
                    if (expandedBox.isVecInside(attackerVec)) {
                        if (0.0 < reachTemp || reachTemp == 0.0) {
                            ...
                            hitVec = ((var19 == null) ? attackerVec : var19.hitVec);
                            reachTemp = 0.0;
                        }
                    } else if (var19 != null) {
            ...
            if (this.pointedEntity != null && (reachTemp < maxReach || this.mc.objectMouseOver == null)) {
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (((LongJump) Client.getModuleManager().get(LongJump.class)).noShake() || Client.getModuleManager().isEnabled(NoHurtCam.class))
            return;
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (movingobjectposition != null) {
                        ...
                        if ((d7 < d3) && !Client.getModuleManager().get(ViewClip.class).isEnabled()) {
                            d3 = d7;
                        }
                    }
                }
    ...
    public void forceUpdateLightmap(float partialTicks) {
        this.lightmapUpdateNeeded = true;
    ...
    public void updateCameraAndRender(float p_181560_1_, long p_181560_2_) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            this.mc.mcProfiler.startSection("EventRenderPreScreen");
            ...
            mc.mcProfiler.startSection("EventRenderGuiLast");
            EventSystem.getInstance(EventRenderGuiLast.class).fire(scaledresolution);
            mc.mcProfiler.endSection();

            mc.mcProfiler.startSection("ClickGui");
            if (Client.instance != null && Client.getClickGui() != null) {
            ...
            mc.mcProfiler.startSection("Notifications");
